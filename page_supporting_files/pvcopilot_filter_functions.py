...
# ═════════════════════════════════════════════════════════════════════════════
# NEW-FILTERS (added 2026-07-15, on trial): stale-data + clipping detection
# via pvanalytics. If these make results worse, delete the two functions below
# and grep "NEW-FILTERS" in pages/pvcopilot.py for the UI/callback wiring.
# ═════════════════════════════════════════════════════════════════════════════
...
# NEW-FILTERS: inverter clipping detection (opt-in checkbox in the UI)
def clipping_filter(df: pd.DataFrame, mapped_variables_dict: dict,
                    slope_max: float = 0.2):
    """
    Flags timestamps where the inverter is clipping (power pinned at its
    limit) using pvanalytics.features.clipping.geometric.

    During clipped periods power no longer responds to irradiance, which
    skews normalized power downward on the best days (injecting fake summer
    seasonality) and makes degradation look milder than it is (a degrading
    array spends less time clipping, so energy falls more slowly than module
    capability). Excluding clipped points keeps the irradiance-responsive
    portion of those same days.

    Detection is geometric — it finds flat-topped stretches in the power
    curve's shape — so no inverter nameplate rating is needed and it works
    on DC or AC power alike.
    ...
    df : pd.DataFrame
        Input dataframe with DatetimeIndex.
    mapped_variables_dict : dict
        Column name mapping; uses the 'DC Power' entry.
    slope_max : float, default 0.2
        Maximum normalized slope for a period to be considered clipped
        (see pvanalytics.features.clipping.geometric). Lower = stricter.

    Returns
    -------
    normal_indices, outlier_indices : pd.Index
    """
    from pvanalytics.features import clipping as pva_clipping

    power_key = mapped_variables_dict.get('DC Power')
    if not power_key or power_key not in df.columns:
        print("  Clipping filter: no power column mapped — skipped")
        return df.index, pd.Index([])

    series = pd.to_numeric(df[power_key], errors='coerce').fillna(0.0)

    # geometric() needs the sampling frequency; infer it, falling back to the
    # median timestep when the index has gaps.
    freq = pd.infer_freq(series.index)
    if freq is None:
        step = series.index.to_series().diff().median()
        if pd.isna(step) or step <= pd.Timedelta(0):
            print("  Clipping filter: cannot infer sampling frequency — skipped")
            return df.index, pd.Index([])
        freq = pd.tseries.frequencies.to_offset(step).freqstr

    clipped = pva_clipping.geometric(ac_power=series, slope_max=slope_max, freq=freq)
    clipped = clipped.fillna(False).astype(bool)
    print(f"  Clipping filter (slope_max={slope_max}) : flagged {clipped.sum()} pts")

    normal_indices  = df.index[~clipped]
    outlier_indices = df.index[clipped]
    return normal_indices, outlier_indices
