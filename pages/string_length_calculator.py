...
layout = dbc.Container([
    ...
    **We would highly appreciate any feedback** (praise, bug reports, 
    ...
    dbc.Row([
        ...
        xs=6,   # phones → 6/12 = half screen
        sm=6,   # small tablets → half
        md=4,   # medium screens
        lg=3    # large screens
    )
...
@app.callback(
    [Output('additional_safety_factor', 'value'),
     Output('additional_safety_factor_loading_div', 'children'),
     ],
...
@app.callback(
    [Output('map', 'figure', allow_duplicate=True),
     Output('map', 'config', allow_duplicate=True)
     ],
    [Input('get-weather', 'n_clicks')],
    [State('lat', 'value'),
     State('lon', 'value')],
...
def get_weather_data(lat, lon):
    ...
    if filename == 'data/124250_37.93_-122.3.npz':
...
def run_simulation(n_clicks, lat, lon, module_parameter_input_type, module_name,
    ...
    info['pvtoolslib version'] = pvtoolslib.__version__
